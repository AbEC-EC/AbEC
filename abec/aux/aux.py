...
def headerReadme(path = "", date = 0):
    text = "AbEC - Ajustable Evolutionary Components\n\n"\
    "This directory contain all the data generated by the experiments.\n"\
    "Each run have your own folder, where you are going to find its data.\n\n"
    return text

def bodyReadme(path = "", date = 0):
    text = f"The date this experiment it was run is: {date['month']:02}/{date['day']:02}/{date['year']} at {date['hour']:02}:{date['minute']:02} \n"\
    "The folders are organized as follows:\n"\
    "\t - [This folder]: Contain a copy of the configuration files used in the experiment:\n"\
    "\t\t - [algoConfig.ini]: \n"\
    "\t\t - [algoConfig.ini]: \n"\
    "\t\t - [algoConfig.ini]: \n"\
    "\t - [results/]: Contain all the results of the runs:\n"\
    "\t\t - [results/results.csv]: Summary of all the runs. \n"\
    "\t\t - [results/XXXX]: Folder of the run XXXX with its data \n"\
    "\t\t - [results/XXXX/<ALGORITHM-NAME>_<YYYYMMDDHHmm>_XXXX_<SEED>_RUN.csv]: \n"\
    "\t\t - [results/XXXX/<ALGORITHM-NAME>_<YYYYMMDDHHmm>_XXXX_<SEED>_OPTIMA.csv]: \n"
    return text

def footerReadme(path = "", date = 0):
